function eliminarRepetidos(array){
    let NoRepetidos = array.filter((elem, index, self) => {
         return  index === self.indexOf(elem);
     })
         return NoRepetidos.sort();
 };
...
Listado.prototype.obtenerCiudades = function() {
    ...
    let arregloCiudades = this.restaurantes.map((restaurant) => restaurant.ubicacion); 
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    return eliminarRepetidos(arregloCiudades);
}
...
Listado.prototype.obtenerRubros = function() {
    let arregloRubros = this.restaurantes.map((restaurant) => restaurant.rubro); 

    return eliminarRepetidos(arregloRubros);
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    let arregloHorariosNuevo = [];

    arregloDeArreglosHorariosObtenidos.forEach((arr) => {
        arr.forEach((horario) => arregloHorariosNuevo.push(horario))
    });
